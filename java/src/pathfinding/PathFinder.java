...
public class PathFinder implements PathFinderInterface {
	 ...
	 */
	public PathFinder(Quinoa quinoa, HeuristicInterface heuristic)
        {
	...
	public ArrayList<Position> findPath(Region region, int maxSearchDistance, Monster mover, int sx, int sy, int tx, int ty)
        {
            //Initialize
            this.region = region;
            nodes = new Node[region.getWidth()][region.getHeight()];
		...
		while (target != nodes[sx][sy]) 
                ...
                Position pos = new Position(sx, sy);
		path.add(0, pos);
	 ...
	 */
	protected boolean isValidLocation(Monster mover, int sx, int sy, int x, int y)
        {
            boolean invalid = (x < 0) || (y < 0) || (x >= region.getWidth()) || (y >= region.getHeight());
